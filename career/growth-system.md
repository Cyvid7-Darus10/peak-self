...
```mermaid
mindmap
  root((High-Income Skills))
    Technical
      AI / Machine Learning
        $130K-$160K, 70% YoY growth
      Cybersecurity
...
```mermaid
flowchart TD
    L["LEVERAGE"] --> A["Labor<br/>People working for you<br/>(oldest form, requires permission)"]
    L --> B["Capital<br/>Money working for you<br/>(requires permission)"]
    L --> C["Products with zero<br/>marginal cost of replication<br/>Code & Media<br/>(PERMISSIONLESS LEVERAGE)"]
...
```mermaid
flowchart TD
    D["Decision"] --> T1["Type 1: One-Way Door<br/>(IRREVERSIBLE)"]
    D --> T2["Type 2: Two-Way Door<br/>(REVERSIBLE)"]
    T1 --> A1["Deep analysis<br/>Broad consultation<br/>Take your time"]
    T2 --> A2["Decide quickly<br/>Small group or individual<br/>Disagree and commit"]
    style T1 fill:#c0392b,stroke:#c0392b,color:#fff
